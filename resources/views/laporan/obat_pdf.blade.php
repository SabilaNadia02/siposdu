            ...
            @forelse ($data as $item)
                ...
                @if (count($obatData) > 0)
                    @foreach ($obatData as $obat)
                        <tr>
                            @if ($loop->first)
                                <td rowspan="{{ count($obatData) }}">{{ $loop->parent->iteration }}</td>
                                <td rowspan="{{ count($obatData) }}">{{ $item->pendaftaran->nama }}</td>
                                <td rowspan="{{ count($obatData) }}">{{ $item->pendaftaran->nik }}</td>
                            @endif
                            <td>{{ $obats[$obat['id_obat']] ?? 'Obat Tidak Dikenal' }}</td>
                            <td>{{ $obat['dosis'] ?? '-' }}</td>
                ...
                @else
                    <tr>
                        ...
                        <td>-</td>
                        <td>{{ $item->keterangan ?? '-' }}</td>
                        <td>{{ date('d/m/Y', strtotime($item->waktu_pemberian)) }}</td>
                    </tr>
                @endif
            @empty
                <tr>
